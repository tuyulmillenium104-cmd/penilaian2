...
const GenerateTab = ({ selectedCampaign }: { selectedCampaign: RallyCampaign | null }) => {
  ...
  const generateContent = useCallback(async () => {
    ...
    try {
      // Get first mission if available
      const firstMission = selectedCampaign?.missions?.[0]
      ...
      const response = await fetch('/api/generate-content', {
        ...
        body: JSON.stringify({ 
          ...
          campaignContext: selectedCampaign ? {
            ...
            knowledgeBase: selectedCampaign.knowledgeBase,
            ...
            missionRules: firstMission?.rules,
            requiredHashtags: selectedCampaign.rules?.match(/#\w+/g)?.join(' '),
            requiredMentions: selectedCampaign.rules?.match(/@\w+/g)?.join(' ')
          } : undefined
        })
      })
      ...
      if (data.success) {
        ...
        if (data.tips) {
          toast.success(`Content generated! Tips: ${data.tips[0]}`)
        } else {
      ...
      <Card className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border-purple-500/30">
        <CardContent className="py-4 px-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h2 className="text-xl font-bold text-purple-400">Elite AI Content Generator</h2>
          </div>
          <p className="text-gray-300 text-sm">Generate konten dengan AI yang menghindari AI-detection dan dioptimasi untuk Rally scoring</p>
          <div className="mt-2 flex flex-wrap gap-2">
            <Badge className="bg-green-500/20 text-green-400 text-xs">Anti AI-Detection</Badge>
            <Badge className="bg-cyan-500/20 text-cyan-400 text-xs">Human Voice</Badge>
            <Badge className="bg-amber-500/20 text-amber-400 text-xs">Rally Optimized</Badge>
          </div>
        </CardContent>
      </Card>
          ...
          <CardContent className="py-3 px-4 space-y-2">
            ...
            {selectedCampaign.knowledgeBase && (
              <div className="text-xs text-gray-400 flex items-center gap-1">
                ...
                <span>Knowledge Base: {selectedCampaign.knowledgeBase.substring(0, 100)}...</span>
              </div>
            )}
            {firstMission && (
              <div className="text-xs text-gray-400 flex items-center gap-1">
                <Target className="w-3 h-3" />
                <span>Mission: {firstMission.title}</span>
              </div>
            )}
            {selectedCampaign.style && (
              <div className="text-xs text-gray-400 flex items-center gap-1">
                <Zap className="w-3 h-3" />
                <span>Style: {selectedCampaign.style.substring(0, 50)}...</span>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-gray-800/50 border-red-500/30">
          <CardContent className="py-3 px-4">
            <div className="flex items-center gap-2 text-red-400">
              ...
              <span className="text-sm">Pilih campaign terlebih dahulu untuk konten yang lebih relevan</span>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <Card className="bg-gray-800/50 border-gray-700">
        ...
        <CardContent className="space-y-3">
          ...
          <Button onClick={generateContent} disabled={isGenerating || !topic.trim()} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700">
            {isGenerating ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Generating...</> : <><Wand2 className="w-4 h-4 mr-2" />Generate Human-Sounding Content</>}
          </Button>
        </CardContent>
      </Card>
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => navigator.clipboard.writeText(generatedContent)} className="border-gray-600 text-gray-300">
                <ClipboardList className="w-4 h-4 mr-1" /> Copy
              </Button>
            </div>
          </CardHeader>
          <CardContent className="space-y-3">
            <pre className="text-sm text-gray-300 whitespace-pre-wrap bg-gray-700/30 p-4 rounded-lg font-sans">{generatedContent}</pre>
            <div className="p-3 bg-amber-500/10 rounded-lg border border-amber-500/30">
              <p className="text-xs text-amber-400 font-medium mb-1">⚠️ BEFORE POSTING:</p>
              <ul className="text-xs text-gray-300 space-y-1">
                <li>• Add required hashtags/mentions from campaign rules</li>
                <li>• Personalize with your own voice/experience</li>
                <li>• Check for any remaining AI-sounding phrases</li>
                <li>• Verify facts against the knowledge base</li>
              </ul>
            </div>
          </CardContent>
        </Card>
